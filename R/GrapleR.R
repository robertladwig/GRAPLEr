...
setMethod(f="GrapleCheckExperimentCompletion",
          ...
          definition=function(grapleObject)
          {
            ...
            grapleObject@StatusCode <- -1
            status <- fromJSON(content(GET(qurl, add_headers(apikey=grapleObject@APIKey)), "text"))
...
setMethod(f="GrapleListPostProcessFilters",
          ...
          definition=function(grapleObject)
          {
            ...
            status <- GET(qurl)
            grapleObject@StatusCode <- 1
            grapleObject@StatusMsg <- paste('The list of post process filters available are :', toString(fromJSON(content(status, "text"))), sep = "")
            return(grapleObject)
          }
)
